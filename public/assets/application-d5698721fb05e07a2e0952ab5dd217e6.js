...
(function() {
  ...
  ready = function() {
    ...
    $.each($('.visual_search').data('query'), function(i, obj) {
      var x;
      visualSearch.searchBox.addFacet("Category", obj);
      x = visualSearch.searchBox.value();
      return visualSearch.searchBox.value(x);
    });
    return $('.edit_customer').submit(function() {
      return $.each(visualSearch.searchQuery.facets(), function(i, obj) {
        var input;
        return input = $("<input>").attr("type", "hidden").attr("name", "customer[categories_raw[" + i + "]]").val(obj.Category.substr(0, 1)).appendTo(".edit_customer");
      });
    });
  };
